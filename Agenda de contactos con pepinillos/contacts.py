...
def ask_yes_or_not(question):
    answer = ""
    while answer != "Si" and answer != "No":
        answer = "{} [Si/No]".format(question)
...
def remove_contact(contacts):
    ...
    for data in found_contacts:
        while not select_contact == data["name"]:
            ...
            select_contact = input("¿Que contacto quieres borrar?")
        ensure_delete = ask_yes_or_not("¿Seguro que quieres borrar el contacto?")
...
def find_contact(contacts):
    ...
    while searching:
        ...
        if len(contact_indexes) > 1:
            ...
            print("No se ha encontrado ninguno.")
            retry_search = ask_yes_or_not("¿Quieres volver a buscar?")

            if retry_search == "Si":
                sleep(1)
                print("\nVolvemos a buscar \n")
            elif retry_search == "No":
                searching = False
